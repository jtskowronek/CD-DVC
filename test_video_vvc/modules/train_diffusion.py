...
class Trainer(object):
    ...
    def load(self, idx=0, load_step=True):
        data = torch.load(
            "/home/user3/huangqiao/diff/epsilonparam/weight/l1_aux0/big-l1-KITTI-d64-t20000-b0.02-vbrFalse-noise-linear-aux0.0/big-l1-KITTI-d64-t20000-b0.02-vbrFalse-noise-linear-aux0.0_0.pt",
        ...
        for epoch in range(self.train_num_steps):
            ...
            if (epoch % self.save_and_sample_every == 0):
                ...
                for i, data in enumerate(iter(self.val_loader)):
                    ...
                    compressed_x,compressed_y, bpp,transmitted_bpp = self.model.compress(
                        img * 2.0 - 1.0,
                        cor_img * 2.0 - 1.0,
                        sample_steps=200,
                        sample_mode="ddim",
                    ...
                    val_transmitted_bpp.append(torch.mean(transmitted_bpp).item())

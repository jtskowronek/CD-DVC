...
lpips_model = lpips.LPIPS(net='vgg')  # 选择'vgg'或者'alex'
...
def pad_images_to_192x192(input_folder, output_folder):
    ...
    for img_name in os.listdir(input_folder):
        ...
        # 创建 192x192 的黑色背景图像
        new_img = Image.new('RGB', (192, 192), (0, 0, 0))  # 黑色背景
...
def pad_and_filter_images(img_path, output_folder):
    """将图片填充到 192x192 并过滤处理"""
    original_size = (176, 144)  # 原始图片大小
    target_size = (192, 192)  # 目标图片大小

    # 处理偶数编号图片：从2开始到148，步长为2
    for i in range(2, 300, 2):
        file_name = f"hall_qcif_{i:05d}.png"  # 文件名格式hall_qcif_00002.png到hall_qcif_00148.png
        ...
        if not os.path.exists(input_path):
            print(f"文件 {input_path} 不存在，跳过该文件。")
            continue

        # 打开图片
        img = Image.open(input_path).convert('RGB')  # 打开为彩色图像

        # 检查图片尺寸是否为176x144
        if img.size != original_size:
            print(f"图片 {file_name} 尺寸不匹配，跳过该文件。")
            continue

        # 创建192x192的黑色背景图像
        new_img = Image.new('RGB', target_size, (0, 0, 0))  # 黑色背景

        # 计算要将原始图像放置在新图像中的位置，使其居中
